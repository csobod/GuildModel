"""`.gmodel` project container round-trip + hand-off (BUILDPLAN M5.1).

A .gmodel holds everything to reopen a project with no external files, plus the
...
from guildmodel.core.project.gmodel import (
    GModelError, save_gmodel, load_gmodel, extract_handoff, HANDOFF_FILES,
)
...
def test_full_round_trip(tmp_path):
    ...
    path = tmp_path / "job.gmodel"
    save_gmodel(path, project=proj, dxf_bytes=b"DXF-BYTES\x00\x01",
...
def test_gdraw_source_and_components_round_trip(tmp_path):
    ...
    from guildmodel.core.project.schema import Component, TempleParams
    ...
    save_gmodel(path, project=proj, gdraw_bytes=b"PK\x03\x04gdraw-zip-bytes")
...
def test_manifest_inventory_and_checksums(tmp_path):
    path = tmp_path / "job.gmodel"
    save_gmodel(path, project=_project(), dxf_bytes=b"abc",
              programs={"posterior_cut.nc": "G21\n"})
    with zipfile.ZipFile(path) as zf:
        manifest = json.loads(zf.read("manifest.json"))
        names = set(zf.namelist())
    assert manifest["format"] == "gmodel" and manifest["format_version"] == 1
...
def test_project_only_save_is_reopenable(tmp_path):
    """A .gmodel saved right after import (no program yet) still reopens."""
    path = tmp_path / "job.gmodel"
    save_gmodel(path, project=_project(), dxf_bytes=b"dxf")
    b = load_gmodel(path)
...
def test_checksum_mismatch_raises(tmp_path):
    ...
    with pytest.raises(GModelError):
        load_gmodel(path)
    # but verify=False still loads it
    assert load_gmodel(path, verify=False).dxf_bytes == b"TAMPERED"


def test_not_a_gmodel_raises(tmp_path):
    bad = tmp_path / "bad.gmodel"
...
def test_extract_handoff_subset(tmp_path):
    path = tmp_path / "job.gmodel"
    save_gmodel(path, project=_project(), dxf_bytes=b"dxf-should-not-be-handed-off",
              programs={"posterior_cut.nc": "G21\nM30\n"},
